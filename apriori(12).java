...
public class Apriori { 
 ...
 private List<Set<String>> generateNextCandidates(List<Set<String>> prevFrequent) {  List<Set<String>> nextCandidates = new ArrayList<>(); 
 int size = prevFrequent.size(); 
 for (int i = 0; i < size; i++) { 
 for (int j = i + 1; j < size; j++) { 
 Set<String> union = new HashSet<>(prevFrequent.get(i));  union.addAll(prevFrequent.get(j)); 
 ...
 List<Set<String>> transactions = Arrays.asList(  Set.of("milk", "bread"), 
 Set.of("milk", "diaper", "beer", "bread"),  Set.of("milk", "diaper", "beer", "cola"), 
 Set.of("diaper", "beer", "bread"), 
 Set.of("milk", "diaper", "bread", "beer") 
 ); 
 Apriori apriori = new Apriori(transactions, 0.6);  List<Set<String>> result = apriori.findFrequentItemsets();  System.out.println("Frequent Itemsets:"); 
